...
#HERE WE TRY TO TURN prob_array_srt.py INTO A WORKABLE INDEPENDENT FUNCTION SO THAT IT CAN BE IMPLEMENTED WITHIN THE OBJECT ORIENTED FRAMEWORK OF preprocessing.py
#ASSUME SUBS HAS BEEN IMPLEMENTED WITHIN THE CLASS AND IS REPRESENTED AS AN OBJECT THERE
...
def pb_array_init(self):
    ...
    self.pb_array = sp.zeros((int(self.last_sub), 3))
    count = sp.arange(int(self.last_sub))
    self.pb_array[:, 0] = count[:]  # an index for each sample starting at 0
    self.pb_array[:, 1] = pb_array[:,0] * 0.01  # 0.01 is the size of the MFCC windows used in play.py, this is the starting point for each window
...
def pb_array_fill(self):
    ...
    while True:
        ...
        if j > len(self.subs) - 1:
            ...
            print(j)
            break
        self.subs_times = tconv(subs, j)  # substimes
        # print(subs_times)
        # print(pb_array[i,1])
